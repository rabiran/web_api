...
describe('Queue tests', () => {
    ...
    describe('Initialization tests', () => {
        ...
        it('should initialize() and close()', async () => {
            const queue = new internal_1.Queue(connection, 'testQ', { durable: false, autoDelete: true });
        ...
        it('should fail to initialize with wrong configuration() ', async () => {
            const queue = new internal_1.Queue(connection, 'testQ', { durable: false, autoDelete: true, prefetch: -1 });
    ...
    describe('Declare queue tests', () => {
        it('should declare queue', async () => {
            ...
            const queue = await internal_1.default.declareQueue(testQueueName, { durable: false, autoDelete: true });
            expect(queue.isInitialized()).toBeTruthy();
            expect(internal_1.default.queue(testQueueName)).toBe(queue);
        });
    ...
    describe('Delete queue tests', () => {
        it('should declare and delete queue', async () => {
            ...
            expect(queue.isInitialized()).toBeTruthy();
            await queue.delete();
            expect(() => internal_1.default.queue(testQueueName)).toThrow();
        });
        it('should declare and delete many queues', async () => {
            for (let i = 0; i < 50; i++) {
                const testQueueName = `testQ${i}`;
                const queue = await internal_1.default.declareQueue(testQueueName, { durable: false, autoDelete: true });
                expect(internal_1.default.queue(testQueueName)).toBeTruthy();
                expect(queue.isInitialized()).toBeTruthy();
                await queue.delete();
                expect(queue.isInitialized()).toBeFalsy();
                expect(() => internal_1.default.queue(testQueueName)).toThrow();
    ...
    describe('Consume tests', () => {
        ...
        it('should consume message', async () => {
            ...
            const sendFinished = new sync_1.Event();
        ...
        it.each([
            ...
            [{ noAck: true }, false, false],
        ])('should handle throwing consumers with consumeOptions %p, and consumer acked %p nacked %p', async (consumeOptions, didConsumerAcked, didConsumerNacked) => {
            const testQueueName = 'testQ';
            const queue = await internal_1.default.declareQueue(testQueueName, { durable: false, autoDelete: true });
            const content = 'Test message';
            const sendFinished = new sync_1.Event();
